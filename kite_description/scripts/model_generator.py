...
def multiple_drones(name='multiple_drones', N=4):
    ...
    for i in range(N):
        ...
        sdfgen.addLine(
            sdfgen.includeDrone("falcon" + str(i), pose_, updateRate=500.0))
    sdfgen.close()
...
def flexible_cable(name='flexible_cable', l=1, r=0.00125, mass=0.1, N=10):
    ...
    sdfgen.addLine(
        sdfgen.addCylinder(link_name="link_0",
                           pose=np.zeros(6),
                           l=body_length,
                           r=r,
                           mass=body_mass))
    for i in range(N - 1):
        ...
        sdfgen.addLine(
            sdfgen.addCylinder(link_name="link_" + str(i + 1),
                               pose=pose_,
                               l=body_length,
                               r=r,
                               mass=body_mass))
        ...
        sdfgen.addLine(
            sdfgen.addUniversalJoint(joint_name,
                                     link1_name,
                                     link2_name,
                                     rel_pose_,
                                     k=k,
...
def payload(name='payload', l=1, r=0.5, mass=3):
    ...
    sdfgen.open()
    sdfgen.addLine(
        sdfgen.addCylinder(link_name="base_link",
                           pose=np.array([0., 0., l / 2., 0., 0., 0.]),
                           l=l,
                           r=r,
                           mass=mass,
                           ixx=Ixx,
...
def payload_flexible_cable(nQ=4, name="mq_rb_fc"):
    ...
    flexible_cable(name='flexible_cable',
                   l=cable_length,
                   r=cable_radius,
                   mass=cable_mass,
                   N=cable_num_links)
    ...
    sdfgen.open()
    sdfgen.addLine(
        sdfgen.includeObject(payload_name, np.zeros(6), payload_name))
    ...
    for i in range(nQ):
        cpose_ = np.array(
            [r * np.cos(th * i), r * np.sin(th * i), z, 0., 0., i * th])
        sdfgen.addLine(
            sdfgen.includeObject("flexible_cable", cpose_,
                                 "flexible_cable_" + colors[i]))
        sdfgen.addLine(
            sdfgen.addBallJoint(
                "payload_cable_joint_" + colors[i],
                payload_name + "::base_link",
                "flexible_cable_" + colors[i] + "::link_0",
                np.array([-0.5 * segment_length, 0., 0., 0., 0., 0.]),
                c=0.01))
        dpose_ = np.array(
            [r2 * np.cos(th * i), r2 * np.sin(th * i), z, 0., 0., 0.])
        ...
        sdfgen.addLine(
            sdfgen.addBallJoint(
                "drone_cable_joint_" + colors[i],
                drone_name + "::base_link",
                "flexible_cable_" + colors[i] + "::link_" +
...
def quad_flexible_cable_payload(name='blue', N=10):
    ...
    sdfgen.addLine(
        sdfgen.addPoseTag(
            np.array(
                [cable_offset_x, cable_offset_y, cable_offset_z, 0., 0., 0.])))
    sdfgen.addLine("</include>")
    ...
    sdfgen.addLine(
        sdfgen.addCylinder(link_name="payload",
                           pose=payload_pose,
                           l=payload_height,
                           r=payload_radius,
                           mass=payload_mass))
    ...
    sdfgen.addLine(
        sdfgen.addUniversalJoint(name="payload_cable_joint",
                                 link1="payload",
                                 link2="link_" + str(N - 1),
                                 pose=np.array(
                                     [half_body_length, 0., 0., 0., 0., 0.]),
    ...
    sdfgen.addLine(
        sdfgen.addBallJoint(name="quadrotor_cable_joint",
                            plink=drone_name + "::base_link",
                            clink="link_0",
                            pose=np.array(
                                [-half_body_length, 0., 0., 0., 0., 0.]),
                            c=0.01))
    ...
    sdfgen.addLine(
        sdfgen.addGroundTruthPlugin(linkName="payload",
                                    topicName=drone_name +
                                    "/payload/odometry/mocap"))
...
def triangle_payload_with_metalplates(name="triangle"):
    ...
    sdfgen.open()
    sdfgen.addLine(
        sdfgen.addTriangularPlank(link_name="base_link",
                                  ixx=0.1666,
                                  iyy=0.1666,
                                  izz=0.1666))
    sdfgen.addLine(
        sdfgen.addTriangularPlank(link_name="link1",
                                  pose=np.array(
                                      [-0.5 + 0.05, 0., 0.05, 0., 0., 0.]),
                                  side=0.1,
                                  material="Red",
                                  h=0.001,
                                  mass=0.1))
    sdfgen.addLine(
        sdfgen.addFixedJoint(joint_name="joint1",
                             parent_link="base_link",
                             child_link="link1"))
    sdfgen.addLine(
        sdfgen.addTriangularPlank(link_name="link2",
                                  pose=np.array(
                                      [0.5 - 0.05, 0., 0.05, 0., 0., 0.]),
                                  side=0.1,
                                  material="Red",
                                  ...
                                  mass=0.1))
    sdfgen.addLine(
        sdfgen.addFixedJoint(joint_name="joint2",
                             parent_link="base_link",
                             child_link="link2"))
    sdfgen.addLine(
        sdfgen.addTriangularPlank(link_name="link3",
                                  pose=np.array(
                                      [0., 0.866 - 0.0866, 0.05, 0., 0., 0.]),
                                  side=0.1,
                                  material="Red",
                                  h=0.001,
                                  mass=0.1))
    sdfgen.addLine(
        sdfgen.addFixedJoint(joint_name="joint3",
                             parent_link="base_link",
                             child_link="link3"))
    sdfgen.close()
    return
...
def plank_payload_with_metalplates(name="plank"):
    ...
    w = 0.02
    sdfgen.addLine(
        sdfgen.addBox(link_name="base_link",
    ...
    dh = 0.001
    sdfgen.addLine(
        sdfgen.addBox(link_name="link1",
                      pose=np.array(
                          [0., -0.5 * (l - dl), 0.5 * (h + dh), 0., 0., 0.]),
                      l=dl,
                      w=dl,
                      material="Red",
                      h=dh,
    ...
    sdfgen.addLine(
        sdfgen.addGroundTruthPlugin(linkName="base_link",
                                    topicName=name +
                                    "_payload/odometry/mocap"))
    sdfgen.close()
    return
...
def square_plank_with_metal_plates(name="square"):
    ...
    sdfgen.addLine(
        sdfgen.addFixedJoint(joint_name="joint3",
                             ...
                             child_link="link3"))
    sdfgen.addLine(
        sdfgen.addFixedJoint(joint_name="joint4",
                             parent_link="base_link",
                             child_link="link4"))
...
def triangle_payload_with_quads():
    ...
    sdfgen.addLine(
        sdfgen.includeObject("triangle_payload",
                             pose=np.array([0., 0., 0., 0., 0., 0.]),
                             name="triangle_payload"))
    ...
    for iter in range(3):
        sdfgen.addLine(
            sdfgen.addCylinder(
                link_name="corner_" + str(iter) + "_link_0",
                pose=np.array([half_body_length, 0., 0., 0., 0.,
                               iter * theta]),
                l=body_length,
                r=r,
                mass=body_mass))
        for i in range(N - 1):
            ...
            pose_[0] = half_body_length + (i + 1) * body_length
            sdfgen.addLine(
                sdfgen.addCylinder(link_name="corner_" + str(iter) + "_link_" +
                                   str(i + 1),
                                   pose=pose_,
                                   l=body_length,
                                   r=r,
