...
def quad_flexible_cable_payload(name='blue', N=10):
    ...
    sdfgen.addline("<include>")
    sdfgen.addline("<uri>model://"+drone_name+"</uri>")
    sdfgen.addline(sdfgen.addPoseTag(np.array([cable_offset_x, cable_offset_y, cable_offset_z, 0., 0., 0.])))
    sdfgen.addline("</include>")
    ...
    sdfgen.addline(sdfgen.addGroundTruthPlugin(linkName="payload", topicName=drone_name+"/payload/odometry/mocap"))
...
def triangle_payload_with_metalplates(name="triangle"):
    ...
    sdfgen.generateConfigFile()
    sdfgen.open()
    sdfgen.addline(sdfgen.addTriangularPlank(link_name="base_link", ixx=0.1666, iyy=0.1666, izz=0.1666))
    sdfgen.addline(sdfgen.addTriangularPlank(link_name="link1", pose=np.array([-0.5+0.05, 0., 0.05, 0., 0., 0.]), side=0.1, material="Red", h=0.001, mass=0.1)) 
    sdfgen.addline(sdfgen.addFixedJoint(joint_name="joint1", parent_link="base_link", child_link="link1"))
    sdfgen.addline(sdfgen.addTriangularPlank(link_name="link2", pose=np.array([0.5-0.05, 0., 0.05, 0., 0., 0.]), side=0.1, material="Red", h=0.001, mass=0.1)) 
    sdfgen.addline(sdfgen.addFixedJoint(joint_name="joint2", parent_link="base_link", child_link="link2"))
    sdfgen.addline(sdfgen.addTriangularPlank(link_name="link3", pose=np.array([0., 0.866-0.0866, 0.05, 0., 0., 0.]), side=0.1, material="Red", h=0.001, mass=0.1))
    sdfgen.addline(sdfgen.addFixedJoint(joint_name="joint3", parent_link="base_link", child_link="link3"))
    sdfgen.close()
    return
...
def plank_payload_with_metalplates(name="plank"):
    ...
    h = 0.02
    w = 0.02
    sdfgen.addline(sdfgen.addBox(link_name="base_link", pose=np.array([0., 0., 0., 0., 0., 0.]) , l=w, w=l, h=h, mass=0.516, ixx=0.1666, iyy=0.1666, izz=0.1666))
    dl = 0.1
    dh = 0.001
    sdfgen.addline(sdfgen.addBox(link_name="link1", pose=np.array([0., -0.5*(l-dl), 0.5*(h+dh), 0., 0., 0.]), l=dl, w=dl, material="Red", h=dh, mass=0.01)) 
    sdfgen.addline(sdfgen.addBox(link_name="link2", pose=np.array([0., 0.5*(l-dl), 0.5*(h+dh), 0., 0.,  0.]), l=dl, w=dl, material="Red", h=dh, mass=0.01)) 
    sdfgen.addline(sdfgen.addFixedJoint(joint_name="joint1", parent_link="base_link", child_link="link1"))
    sdfgen.addline(sdfgen.addFixedJoint(joint_name="joint2", parent_link="base_link", child_link="link2"))

    sdfgen.addline(sdfgen.addGroundTruthPlugin(linkName="base_link", topicName=name+"_payload/odometry/mocap"))
    sdfgen.close()
    return
...
def square_plank_with_metal_plates(name="square"):
    ...
    h = 0.02
    w = 1.22
    sdfgen.addline(sdfgen.addBox(link_name="base_link", l=l, w=w, h=h, ixx=0.1666, iyy=0.1666, izz=0.1666, mass=1))
    dl = 0.1
    dh = 0.001
    sdfgen.addline(sdfgen.addBox(link_name="link1", pose=np.array([0.5*(l-dl), 0.5*(w-dl), 0.5*(h+dh), 0., 0., 0.]), l=dl, w=dl, material="Red", h=dh, mass=0.01)) 
    sdfgen.addline(sdfgen.addBox(link_name="link2", pose=np.array([-0.5*(l-dl), 0.5*(w-dl), 0.5*(h+dh), 0., 0., 0.]), l=dl, w=dl, material="Red", h=dh, mass=0.01)) 
    sdfgen.addline(sdfgen.addBox(link_name="link3", pose=np.array([-0.5*(l-dl), -0.5*(w-dl), 0.5*(h+dh), 0., 0., 0.]), l=dl, w=dl, material="Red", h=dh, mass=0.01)) 
    sdfgen.addline(sdfgen.addBox(link_name="link4", pose=np.array([0.5*(l-dl), -0.5*(w-dl), 0.5*(h+dh), 0., 0., 0.]), l=dl, w=dl, material="Red", h=dh, mass=0.01)) 
    sdfgen.addline(sdfgen.addFixedJoint(joint_name="joint1", parent_link="base_link", child_link="link1"))
    sdfgen.addline(sdfgen.addFixedJoint(joint_name="joint2", parent_link="base_link", child_link="link2"))
    sdfgen.addline(sdfgen.addFixedJoint(joint_name="joint3", parent_link="base_link", child_link="link3"))
    sdfgen.addline(sdfgen.addFixedJoint(joint_name="joint4", parent_link="base_link", child_link="link4"))
...
def triangle_payload_with_quads():
    ...
    sdfgen.addline(sdfgen.includeObject("triangle_payload", pose=np.array([0., 0., 0., 0., 0., 0.]), name="triangle_payload"))
...
if __name__ == '__main__':
    ...
    # payload("rigid_payload", l=0.25, r=0.5)
    # payload_flexible_cable(nQ=4)
    # quad_flexible_cable_payload(name='red')
    # quad_flexible_cable_payload(name='white')
    # triangle_payload_with_metalplates()
    # quad_flexible_cable_payload(name='blue', N=10)
    # quad_flexible_cable_payload(name='red')
    # quad_flexible_cable_payload(name='white')
    # quad_flexible_cable_payload(name='yellow')
    # plank_payload_with_metalplates()
    # square_plank_with_metal_plates()
    payload_flexible_cable(nQ=3, name="payload_with_three_quads")
    # triangle_payload_with_quads()
    pass
