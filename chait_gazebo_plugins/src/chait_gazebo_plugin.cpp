...
CHAITGazeboPlugin::CHAITGazeboPlugin(std::string name)
    : gazebo::ModelPlugin(), plugin_name_(std::move(name)), nh_(nullptr) {}

CHAITGazeboPlugin::~CHAITGazeboPlugin() {
...
void CHAITGazeboPlugin::loadAndReadDefaultParams(gazebo::physics::ModelPtr _model,
                                          sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("linkName")) {
    ...
    gzerr << "[CHAITGazeboPlugin] Please specify a linkName\n";
...
void CHAITGazeboPlugin::Load(gazebo::physics::ModelPtr _model,
                             sdf::ElementPtr _sdf) {
  ...
  updateConnection_ = gazebo::event::Events::ConnectWorldUpdateBegin(
      boost::bind(&CHAITGazeboPlugin::OnUpdate, this, _1));

  gzmsg << "[" << plugin_name_ << "] ... plugin loaded!";
}

void CHAITGazeboPlugin::OnUpdate(const gazebo::common::UpdateInfo &_info) {
...
void CHAITGazeboPlugin::run() {}

GZ_REGISTER_MODEL_PLUGIN(CHAITGazeboPlugin);
